...
public class ByteArrayToString {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		byte[] array2 = "땡큐".getBytes(Charset.defaultCharset()); 	//땡큐 --> byte변환해서 배열에 저장
		byte[] array3 = "베리".getBytes(Charset.forName("MS949"));	//예외발생 : 실행시 예외
		byte[] array4 = "땡큐".getBytes("UTF-8");		//<-- 예외 처리 필요: 일반예외(컴파일시 예외 발생)>>
		
		//2. 각 인코딩의 바이트 수 출력
		System.out.println(array1.length); //MS949 : 한글1자 2byte, 	4
		System.out.println(array2.length); //MS949 : 한글1자 2		4
		System.out.println(array3.length); //MS949 : 한글1자 2		4
		System.out.println(array4.length); //UTF-8 : 한글1자 3		6
